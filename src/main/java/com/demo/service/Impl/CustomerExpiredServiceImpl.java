        ...
        if (customerInvoices != null) {
            for (Customer_Invoice ci : customerInvoices) {
                ...
                if (end_month == current_month) {
                    expired = (current_hours - end_hours);
                    fine = expired * 10;
                    if(current_day > end_day || (current_day == end_day && current_hours > end_hours))
                        warning = true;
                } else if (end_month < current_month) {
                    expired = Math.abs(current_day - end_day) + (current_month - end_month) * 24;
                    fine = expired * 10;
                    warning = true;
                }
                if (warning == true) {
                    String current_time = current_date.getHours() + ":" + current_date.getMinutes();
                    ExpiredResponse ex = new ExpiredResponse(ci.getPayment_c().getBooking().getCustomer().getIdUser()
                            , current_date
                            , current_time
                            , end_date
                            , end_time
